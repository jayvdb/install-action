...
use install_action_internal_codegen::{
    cargo_manifest, crates_io, download, github, github_download, replace_vars, workspace_root,
    BaseManifest, HostPlatform, Manifest, ManifestDownloadInfo, ManifestRef, ManifestTemplate,
    ManifestTemplateDownloadInfo, Manifests, Signing, SigningKind, Version,
};
...
fn main() -> Result<()> {
    ...
    for page in 1.. {
        let per_page = 100;
        let mut r: github::Releases = github_download(&format!(
